...
class CommonScaffold extends StatelessWidget {
  const CommonScaffold({super.key,this.child,this.appBar,this.showDrawer=true});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: appBar??AppBar(
        ...
        centerTitle: false,
        backgroundColor: colorBg,
        title: loadAssetImage(path: icLoginLogo,width: size.width*zero29),
        actions: [
          ...
          PopupMenuButton<int>(
            ...
            itemBuilder: (context) => [
              _buildPopupMenuItem(context: context,index:  0, text: 'Profile'),
              _buildPopupMenuItem(context: context, index: 1, text: 'User'),
              _buildPopupMenuItem(context: context,index:  2, text: 'System Logs'),
              _buildPopupMenuItem(context: context,index:  2, text: 'Sign out'),
            ],
            child: Row(
              children: [
                CommonTextWidget(text: "Sameer Khan",style: commonTextStyle(fontSize: twelve),),
        ...
        child: Consumer<DashboardProvider>(
          builder: (context, hoverProvider, child) {
            return Container(
              ...
              child: CommonTextWidget(text: text,style: commonTextStyle(
                fontSize: 12,fontWeight: FontWeight.w400,
                color: hoverProvider.hoveredIndex == index ? colorButton : Colors.black,
              ),),
            );
          },
        ),
